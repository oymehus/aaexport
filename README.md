...
* **Canonical Area Paths:** Resolves each work item's stable Area ID against the current project classification tree, avoiding stale paths after Area nodes are moved or renamed.
...
*Note on moved Area Paths:* Azure DevOps can retain an old `System.AreaPath` value on work items after an Area node is moved or renamed. This is most visible for states such as Removed or Rejected that are excluded from backlog and board processing. AAExport uses `System.AreaId` and the current classification tree as the authoritative source, so Area Path, hierarchy levels, and node name remain current for every state, including rows reused by incremental exports.
