...
* **Canonical Area Paths:** Re-resolves Area Path from the project classification tree using each work item's stable Area ID, so incremental exports self-heal after Area nodes are moved or renamed.
...
*Note on moved Area Paths:* When an Area node is moved or renamed, existing exports can retain the old path. Items in states such as Removed or Rejected make this most visible, because they are excluded from backlog and board processing. Incremental exports are affected too, since a node move does not change a work item's `Changed Date`, so reused rows would otherwise keep a stale path forever. AAExport re-resolves each reused row against the current classification tree using the work item's stable `System.AreaId`, so Area Path, hierarchy levels, and node name self-heal on the next run for every state.
